...
const std::string test_data_dir = "../tests/test_data/";

TEST(ConverterJSONTest, LoadValidConfig) {
    std::string error;
    ConverterJSON conv;

    const std::string config_path = test_data_dir + "test_config.json";
    const std::string requests_path = test_data_dir + "test_requests.json";
    ...
    std::ifstream file1(test_data_dir + "doc1.txt");
...
TEST(ConverterJSONTest, LoadInvalidConfig) {
    std::string error;
    ConverterJSON conv;
    const std::string invalid_path = test_data_dir + "bad_config.json";
...
TEST(ConverterJSONTest, GetResponsesLimitDefaultAndCustom) {
    ...
    {
        ConverterJSON conv;
        const std::string path = test_data_dir + "test_config.json";
    ...
    {
        ConverterJSON conv;
        const std::string path = test_data_dir + "config_with_max.json";
        ASSERT_TRUE(conv.LoadConfig(path, error)) << error;
